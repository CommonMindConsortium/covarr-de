---
title: "HBCC - sgACC - FEATURECOUNTS - GRCh38 - CQN"
...
knit2synapse::createAndKnitToFolderEntity(file = "individual-cohorts/HBCC_sgACC_FEATURECOUNTS_GRCh38_CQN.Rmd",
                                          parentId ="syn22012942",
                                          folderName = 'HBCC - sgACC - DREAM - FEATURECOUNTS - GRCh38 - CQN')
```
...
activityDescription = 'Covariate analysis of GRCh38 feature counts with CQN normalisation (sgACC)';

thisFileName <- 'HBCC_sgACC_FEATURECOUNTS_GRCh38_CQN.Rmd'
...
metadata = downloadFile_version(METADATA_QC_ACC_ID, version = 20) %>%
  ...
  dplyr::filter(`rnaSeq_dissection:Brain_Region` %in% c("sgACC")) %>% 
...
phase2 <- mvIC::mvForwardStepwise(exprObj = CQN.GENE_EXPRESSION$E,
                         ...
                         data = COVARIATES,
                         variables = array(c("(1|LibraryBatch)", "(1|IsolationBatch)")))
...
# Code
CODE <- Folder(name = "HBCC - sgACC - DREAM - FEATURECOUNTS - GRCh38 - CQN", parentId = parentId)
CODE <- synStore(CODE)

# Store covariates
COVARIATES = rownameToFirstColumn(COVARIATES, 'SampleID')
write.table(COVARIATES, file = 'CMC_HBCC_sgACC_Covariates.tsv', sep = '\t', row.names=F, quote=F)
COV_OBJ = synapser::File('CMC_HBCC_sgACC_Covariates.tsv', name = 'Covariates', parentId = CODE$properties$id)
...
PROCESSED_COUNTS$filteredExprMatrix$counts %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_sgACC_Counts.tsv', sep = '\t', row.names=F, quote=F)
COUNT_OBJ = File('CMC_HBCC_sgACC_Counts.tsv', name = 'Counts (filtered raw)', parentId = CODE$properties$id)
...
CQN.GENE_EXPRESSION$E %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_sgACC_logCPM.tsv', sep = '\t', row.names=F, quote=F)
LCOUNT_OBJ = File('CMC_HBCC_sgACC_logCPM.tsv', name = 'Counts (filtered logCPM)', parentId = CODE$properties$id)
...
CQN.GENE_EXPRESSION$offset %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_sgACC_offset.tsv', sep = '\t', row.names=F, quote=F)
OFFSET_OBJ = File('CMC_HBCC_sgACC_offset.tsv', name = 'Gene length and GC content offset', parentId = CODE$properties$id)
