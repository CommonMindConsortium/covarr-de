...
df_one <- inner_join(df_one, dplyr::select(gandal, gene_id,  `Gandal, BD t-statistic`,`Gandal, ASD t-statistic`), by = c("gene_id")) %>% 
  group_by(gene_id) %>% 
...
Heatmap(as.matrix(cor$r), 
        ...
        grid.text(sprintf("%.1f", as.matrix(cor$r)[i, j]), x, y, gp = gpar(fontsize = 8))
          }
        )
```
...
df_two <- inner_join(df_two, dplyr::select(gandal, gene_id, `Gandal, SCZ t-statistic`), by = c("gene_id")) %>% 
  group_by(gene_id) %>% 
  dplyr::filter(row_number()==1) %>% 
  tibble::column_to_rownames(var = "gene_id")
...
df_three <- df_three %>% 
  separate(gene_id, c("gene_id", "transcript")) %>% 
  dplyr::select(-one_of("transcript"))
df_three <- inner_join(df_three, dplyr::select(gandal, gene_id,  `Gandal, BD t-statistic`,`Gandal, ASD t-statistic`), by = c("gene_id")) %>% 
  group_by(gene_id) %>% 
  dplyr::filter(row_number()==1) %>% 
  tibble::column_to_rownames(var = "gene_id")
