---
title: "Meta-analysis on diagnosis and sex in MSSM-Penn-Pitt and HBCC ACC/DLPFC"
author: "KM and GH"
date: "`r date()`"
...
knit2synapse::createAndKnitToFolderEntity(file = "individual-cohorts/meta-analysis.Rmd",
                                          ...
                                          folderName = 'Meta-analysis on diagnosis and sex')
```
...
```{r input_data}
mpp_acc <- readr::read_tsv(synGet("syn22276990")$path) %>% 
  dplyr::select(Comparison, gene_id, AveExpr, t, logFC, P.Value, adj.P.Val)
mpp_dlpfc <- readr::read_tsv(synGet("syn22277179")$path) %>% 
  dplyr::select(Comparison, gene_id, AveExpr, t, logFC, P.Value, adj.P.Val)
hbcc_dsACC <- readr::read_tsv(synGet("syn22280364")$path) %>% 
  dplyr::select(Comparison, gene_id, AveExpr, t, logFC, P.Value, adj.P.Val) 
hbcc_dlpfc <- readr::read_tsv(synGet("syn22280045")$path) %>% 
  dplyr::select(Comparison, gene_id, AveExpr, t, logFC, P.Value, adj.P.Val)
```
...
dlpfcList <- make_list(mpp_dlpfc, hbcc_dlpfc, string = "Control_Female_vs_Control_Male")
accList <- make_list(mpp_acc, hbcc_dsACC, string = "Control_Female_vs_Control_Male")
...
dlpfcList <- make_list(mpp_dlpfc, hbcc_dlpfc, string = "SCZ_Female_vs_SCZ_Male")
accList <- make_list(mpp_acc, hbcc_dsACC, string = "SCZ_Female_vs_SCZ_Male")
...
dlpfcList <- make_list(mpp_dlpfc, hbcc_dlpfc, string = "Other_Female_vs_Other_Male")
accList <- make_list(mpp_acc, hbcc_dsACC, string = "Other_Female_vs_Other_Male")
...
dlpfcList <- make_list(mpp_dlpfc, hbcc_dlpfc, string = "Control_Female_vs_Other_FemaleSCZ_Female")
accList <- make_list(mpp_acc, hbcc_dsACC, string = "Control_Female_vs_Other_FemaleSCZ_Female")
...
dlpfcList <- make_list(mpp_dlpfc, hbcc_dlpfc, string = "Control_Male_vs_Other_MaleSCZ_Male")
accList <- make_list(mpp_acc, hbcc_dsACC, string = "Control_Male_vs_Other_MaleSCZ_Male")
