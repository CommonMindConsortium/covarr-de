---
title: "Meta-analysis on Sex*Dianosis interaction in MSSM-Penn-Pitt and HBCC ACC/DLPFC"
author: "KM and GH"
date: "`r date()`"
output:
  html_document:
...
# rmarkdown::render("individual-cohorts/meta_analysis/meta-analysis_interaction.Rmd")
...
createAndKnitToFolderEntity(file = "individual-cohorts/meta_analysis/meta-analysis_interaction.Rmd",
                                          parentId ="syn22315403",
                                          folderName = 'Meta-analysis on Sex*Diagnosis interaction',
                                          overwrite=TRUE, 
                                          knitmd=TRUE)
```
...
thisFileName <- 'meta-analysis_interaction.Rmd'
...
hbcc_dlpfc <- readr::read_tsv(synGet("syn22280364")$path) %>% 
  dplyr::select(Comparison, gene_id, AveExpr, t, logFC, P.Value, adj.P.Val) %>% getGeneSymbol( 'gene_id' )

Ids <- c("syn22276990", "syn22277179", "syn22086829", "syn22280364")
```
...
```{r store, eval=FALSE}
# Code
CODE <- Folder(name = "Meta-analysis on Sex*Diagnosis interaction", parentId = "syn22315403") # Make sure these match the folder name and parent Id in the first chunk.
CODE <- synStore(CODE)

# Store meta-analysis
res_dlpfc %>%
  tibble::rownames_to_column(var = "gene_id") %>% 
  write.table(file = 'CMC_dlpfc_interaction_meta_analysis.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('CMC_dlpfc_interaction_meta_analysis.tsv', 
                name = 'Interaction meta-analysis (DLPFC)', 
                parentId = CODE$properties$id)
nEXP_OBJ = synStore(nEXP_OBJ, used = Ids, 
                    executed = thisFile)
res_acc %>%
  tibble::rownames_to_column(var = "gene_id") %>% 
  write.table(file = 'CMC_acc_Interaction_meta_analysis.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('CMC_acc_Interaction_meta_analysis.tsv', 
                name = 'Interaction meta-analysis (ACC)', 
                parentId = CODE$properties$id)
nEXP_OBJ = synStore(nEXP_OBJ, used = Ids, 
                    executed = thisFile)
```
