---
title: "HBCC - STG - FEATURECOUNTS - GRCh38 - CQN"
...
knit2synapse::createAndKnitToFolderEntity(file = "individual-cohorts/HBCC_STG_FEATURECOUNTS_GRCh38_CQN.Rmd",
                                          parentId ="syn22012942",
                                          folderName = 'HBCC - STG - DREAM - FEATURECOUNTS - GRCh38 - CQN')
```
...
activityDescription = 'Covariate analysis of GRCh38 feature counts with CQN normalisation (STG)';

thisFileName <- 'HBCC_STG_FEATURECOUNTS_GRCh38_CQN.Rmd'
...
METADATA_QC_STG_ID = 'syn16816488' 
ALL_USED_IDs = METADATA_QC_STG_ID
metadata = downloadFile_version(METADATA_QC_STG_ID, version = 20) %>%
  ...
  dplyr::filter(`rnaSeq_dissection:Brain_Region` %in% c("STG"))

## Download reprocessed counts (STG)
counts = 'syn21867866'
...
# Code
CODE <- Folder(name = "HBCC - STG - DREAM - FEATURECOUNTS - GRCh38 - CQN", parentId = parentId)
CODE <- synStore(CODE)

# Store covariates
COVARIATES = rownameToFirstColumn(COVARIATES, 'SampleID')
write.table(COVARIATES, file = 'CMC_HBCC_STG_Covariates.tsv', sep = '\t', row.names=F, quote=F)
COV_OBJ = synapser::File('CMC_HBCC_STG_Covariates.tsv', name = 'Covariates', parentId = CODE$properties$id)
...
PROCESSED_COUNTS$filteredExprMatrix$counts %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_STG_Counts.tsv', sep = '\t', row.names=F, quote=F)
COUNT_OBJ = File('CMC_HBCC_STG_Counts.tsv', name = 'Counts (filtered raw)', parentId = CODE$properties$id)
...
CQN.GENE_EXPRESSION$E %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_STG_logCPM.tsv', sep = '\t', row.names=F, quote=F)
LCOUNT_OBJ = File('CMC_HBCC_STG_logCPM.tsv', name = 'Counts (filtered logCPM)', parentId = CODE$properties$id)
...
CQN.GENE_EXPRESSION$offset %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'CMC_HBCC_STG_offset.tsv', sep = '\t', row.names=F, quote=F)
OFFSET_OBJ = File('CMC_HBCC_STG_offset.tsv', name = 'Gene length and GC content offset', parentId = CODE$properties$id)
