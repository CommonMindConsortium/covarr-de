---
title: "Differential Expression - HBCC - dsACC - Dx"
...
knit2synapse::createAndKnitToFolderEntity(file = "individual-cohorts/split_by_analysis/HBCC_dsACC_DE_Diagnosis.Rmd",
...
adjust.covars <- c("Dx", "RIN", "IntronicRate", "IntragenicRate", "IntergenicRate", "ageOfDeath")
random_effect <- c("Reported_Gender", "IsolationBatch")
```
...
RESIDUAL.GENE_EXPRESSION %>%
  ...
  write.table(file = 'CMC_dsACC_Dx_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('CMC_dsACC_Dx_netResidualExpression.tsv', 
...
write.table(DE, file = 'CMC_dsACC_Dx_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = File('CMC_dsACC_Dx_DiffExpression.tsv', 
...
saveRDS(ADJUSTED.FIT, file = 'CMC_dsACC_Dx_dream-output.rds')
ADJ_OBJ = File('CMC_dsACC_Dx_dream-output.rds',
...
saveRDS(sortCols(violin_plot), file = 'CMC_dsACC_Dx_variancePartition.rds')
ADJ_OBJ = File('CMC_dsACC_Dx_variancePartition.rds',
...
# Store voom object
saveRDS(VOOM.GENE_EXPRESSION, file = 'CMC_dsACC_Dx_voom.rds')
ADJ_OBJ = File('CMC_dsACC_Dx_voom.rds',
               name = 'voom object', 
               parentId = CODE$properties$id)
ADJ_OBJ = synStore(ADJ_OBJ, used = ALL_USED_IDs, activityName = activityName,
                   executed = thisFile, activityDescription = activityDescription)
...
# Store contrasts
saveRDS(L, file = 'CMC_dsACC_Dx_contrasts.rds')
ADJ_OBJ = File('CMC_dsACC_Dx_contrasts.rds',
...
# Store contrasts
saveRDS(formula, file = 'CMC_dsACC_Dx_formula.rds')
ADJ_OBJ = File('CMC_dsACC_Dx_formula.rds',
               name = 'formula', 
               parentId = CODE$properties$id)
ADJ_OBJ = synStore(ADJ_OBJ, used = ALL_USED_IDs, activityName = activityName,
                   executed = thisFile, activityDescription = activityDescription)
