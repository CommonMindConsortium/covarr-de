---
title: "Differential Expression - HBCC - dsACC - Sex"
...
knit2synapse::createAndKnitToFolderEntity(file = "individual-cohorts/split_by_analysis/HBCC_dsACC_DE_Sex.Rmd",
...
adjust.covars <- c("Sex", "Dx", "RIN", "IntronicRate", "IntragenicRate", "IntergenicRate", "ageOfDeath")
...
RESIDUAL.GENE_EXPRESSION %>%
  ...
  write.table(file = 'CMC_dsACC_Sex_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('CMC_dsACC_Sex_netResidualExpression.tsv', 
...
write.table(DE, file = 'CMC_dsACC_Sex_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = File('CMC_dsACC_Sex_DiffExpression.tsv', 
...
saveRDS(ADJUSTED.FIT, file = 'CMC_dsACC_Sex_dream-output.rds')
ADJ_OBJ = File('CMC_dsACC_Sex_dream-output.rds',
...
saveRDS(sortCols(violin_plot), file = 'CMC_dsACC_Sex_variancePartition.rds')
ADJ_OBJ = File('CMC_dsACC_Sex_variancePartition.rds',
...
# Store voom object
saveRDS(VOOM.GENE_EXPRESSION, file = 'CMC_dsACC_Sex_voom.rds')
ADJ_OBJ = File('CMC_dsACC_Sex_voom.rds',
               name = 'voom object', 
               parentId = CODE$properties$id)
ADJ_OBJ = synStore(ADJ_OBJ, used = ALL_USED_IDs, activityName = activityName,
                   executed = thisFile, activityDescription = activityDescription)
...
# Store contrasts
saveRDS(L, file = 'CMC_dsACC_Sex_contrasts.rds')
ADJ_OBJ = File('CMC_dsACC_Sex_contrasts.rds',
...
# Store contrasts
saveRDS(formula, file = 'CMC_dsACC_Sex_formula.rds')
ADJ_OBJ = File('CMC_dsACC_Sex_formula.rds',
               name = 'formula', 
               parentId = CODE$properties$id)
ADJ_OBJ = synStore(ADJ_OBJ, used = ALL_USED_IDs, activityName = activityName,
                   executed = thisFile, activityDescription = activityDescription)
