---
title: "Differential Expression - MSSM-Penn-Pitt - ACC - Sex"
...
adjust.covars <- c("Sex", "ageOfDeath", "RIN2", "RIN", "rRNARate", "IntronicRate", "IntragenicRate", "IntergenicRate")
...
Calculate weighted residuals and add back "Reported_Gender" to the residuals
```{r varsToAddBack}
...
COVARIATES.tmp = data.matrix(COVARIATES[,c('Sex','Institution')])
...
WGCNA::plotDendroAndColors(tree, 
                           ...
                           groupLabels = colnames(COVARIATES.tmp[,c('Sex','Institution')]))
```
...
### Differential expression analysis (Female - Male)
...
RESIDUAL.GENE_EXPRESSION %>%
  ...
  write.table(file = 'CMC_ACC_Sex_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('CMC_ACC_Sex_netResidualExpression.tsv', 
...
write.table(DE, file = 'CMC_ACC_Sex_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = File('CMC_ACC_Sex_DiffExpression.tsv', 
...
saveRDS(ADJUSTED.FIT, file = 'CMC_ACC_Sex_dream-output.rds')
ADJ_OBJ = File('CMC_ACC_Sex_dream-output.rds',
...
saveRDS(sortCols(violin_plot), file = 'CMC_ACC_Sex_variancePartition.rds')
ADJ_OBJ = File('CMC_ACC_Sex_variancePartition.rds',
...
# Store voom object
saveRDS(VOOM.GENE_EXPRESSION, file = 'CMC_ACC_Sex_voom.rds')
ADJ_OBJ = File('CMC_ACC_Sex_voom.rds',
               name = 'voom object', 
               parentId = CODE$properties$id)
ADJ_OBJ = synStore(ADJ_OBJ, used = ALL_USED_IDs, activityName = activityName,
                   executed = thisFile, activityDescription = activityDescription)
...
# Store contrasts
saveRDS(L, file = 'CMC_ACC_Sex_contrasts.rds')
ADJ_OBJ = File('CMC_ACC_Sex_contrasts.rds',
...
# Store contrasts
saveRDS(formula, file = 'CMC_ACC_Sex_formula.rds')
ADJ_OBJ = File('CMC_ACC_Sex_formula.rds',
               name = 'formula', 
               parentId = CODE$properties$id)
ADJ_OBJ = synStore(ADJ_OBJ, used = ALL_USED_IDs, activityName = activityName,
                   executed = thisFile, activityDescription = activityDescription)
