---
title: "Differential Expression - HBCC - STG - Dx"
...
knit2synapse::createAndKnitToFolderEntity(file = "individual-cohorts/split_by_analysis/HBCC_STG_DE_Diagnosis.Rmd",
                                 parentId ="syn22086820",
...
adjust.covars <- c("Dx", "RIN2", "IntronicRate", "IntragenicRate", "IntergenicRate", "ageOfDeath")
...
RESIDUAL.GENE_EXPRESSION %>%
  ...
  write.table(file = 'CMC_STG_Dx_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
nEXP_OBJ = File('CMC_STG_Dx_netResidualExpression.tsv', 
...
write.table(DE, file = 'CMC_STG_Dx_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
DEXP_OBJ = File('CMC_STG_Dx_DiffExpression.tsv', 
...
saveRDS(ADJUSTED.FIT, file = 'CMC_STG_Dx_dream-output.rds')
ADJ_OBJ = File('CMC_STG_Dx_dream-output.rds',
...
saveRDS(sortCols(violin_plot), file = 'CMC_STG_Dx_variancePartition.rds')
ADJ_OBJ = File('CMC_STG_Dx_variancePartition.rds',
...
# Store voom object
saveRDS(VOOM.GENE_EXPRESSION, file = 'CMC_STG_Dx_voom.rds')
ADJ_OBJ = File('CMC_STG_Dx_voom.rds',
               name = 'voom object', 
               parentId = CODE$properties$id)
ADJ_OBJ = synStore(ADJ_OBJ, used = ALL_USED_IDs, activityName = activityName,
                   executed = thisFile, activityDescription = activityDescription)
...
# Store contrasts
saveRDS(L, file = 'CMC_STG_Dx_contrasts.rds')
ADJ_OBJ = File('CMC_STG_Dx_contrasts.rds',
...
# Store contrasts
saveRDS(formula, file = 'CMC_STG_Dx_formula.rds')
ADJ_OBJ = File('CMC_STG_Dx_contrasts.rds',
               name = 'formula', 
               parentId = CODE$properties$id)
ADJ_OBJ = synStore(ADJ_OBJ, used = ALL_USED_IDs, activityName = activityName,
