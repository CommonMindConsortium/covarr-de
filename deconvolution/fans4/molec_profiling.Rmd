...
res_cv_FANS = lapply(CTarray, function(CT){
  ...
  geneTab = AnnotationDbi::select(ensdb, keys=names(isexpr[isexpr]), columns=c("GENEID", "GENEBIOTYPE"), keytype="GENEID")
...
df_de_summary = lapply( 1:length(tabRampList), function(j){
  ...
  df_de_summary = lapply( names(tabList), function(key){
    ...
    geneTab = AnnotationDbi::select(ensdb, keys=rownames(tab), columns=c("GENEID", "GENEBIOTYPE"), keytype="GENEID")
