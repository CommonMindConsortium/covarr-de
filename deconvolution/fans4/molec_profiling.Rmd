...
# Gene set analysis
```{r zenith.celltype, cache=FALSE}

geneSets_GO = get_GeneOntology("MF")

# Map from Ensembl genes in geneSets_GO to 
# Ensembl names from RNA-seq data 
index = ids2indices( recodeToList(geneSets_GO), rownames(fitBasic))
...
df_CT_zenith = lapply( colnames(coef(fitBasic)), function(key){
  res = zenith( fitBasic, coef = key, index = idxGO)
...
df_gs = lapply( df_CT_zenith[,unique(coef)], function(key){
  ...
  df_CT_zenith[(coef == key),][order(p.greater),][1:4,]
  })
...
```
  ...
  res = zenith( fitBasic, coef = 'CellTypeGLU', index = idxGO)
