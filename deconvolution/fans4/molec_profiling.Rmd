...
L = cbind(L, data.frame(GABAGLU.Other = c(1, 1, -1, -1),
                    MgAs.Other = c(-1/3, -1/3, -1/3, 1),
                    Olig.Other = c(-1/3, -1/3, 1, -1/3) ))

fitBasic = dream( vobj, form, info, L=L, BPPARAM=SnowParam(12, progressbar=TRUE))
```

idx = as.integer(seq(1, 2000, length.out=100))
lapply(2:length(idx), function(i){
  message(i, '     ')
fitBasic = dream( vobj[idx[i-1]:idx[i],], form, info, L=L, BPPARAM=SerialParam())

})

i = 173:173
fitBasic = dream( vobj[i,,drop=FALSE], form, info, L=L, BPPARAM=SerialParam())
...
```{r plotContrasts}
plotContrasts(round(L,2)) + theme(axis.text.x=element_text(angle=45, hjust=1))   
```
...
res_cv_FANS = lapply(CTarray, function(CT){
  ...
  geneTab = select(ensdb, keys=names(isexpr[isexpr]), columns=c("GENEID", "GENEBIOTYPE"), keytype="GENEID")
...
df_de_summary = lapply( 1:length(tabRampList), function(j){
  ...
  df_de_summary = lapply( names(tabList), function(key){
    ...
    geneTab = select(ensdb, keys=rownames(tab), columns=c("GENEID", "GENEBIOTYPE"), keytype="GENEID")
