...
# Gene set analysis
```{r zenith.celltype, cache=FALSE, eval=FALSE}

geneSets_GO = get_GeneOntology()

filter_gs = function(fit, gs){
  # Map from Ensembl genes in geneSets_GO to 
  # Ensembl names from RNA-seq data 
  index = ids2indices( recodeToList(gs), rownames(fit))

  # filter by size of gene set
  n_genes_in = 15
  setSize = sapply(index, length)
  idxGO = index[(setSize > n_genes_in) & (setSize < 500)]
  idxGO
}

fit = fitBasic[fitBasic$Amean > -Inf,]
idxGO = filter_gs( fit, geneSets_GO)

df_CT_zenith = lapply( colnames(coef(fitBasic)), function(key){
  res = zenith( fit, coef = key, index = idxGO, inter.gene.cor=0.01)
  data.table(Geneset = rownames(res), res, coef = key)
})
df_CT_zenith = do.call(rbind, df_CT_zenith)

df_gs = lapply( df_CT_zenith[,unique(coef)], function(key){

  # results for this coef
  df_sub = df_CT_zenith[(coef == key),][order(p.greater),]

  # remove gene sets with small p-values from other tests
  df_sub2 = df_CT_zenith[(coef != key),][order(p.greater),]
  df_sub2 = df_sub2[,.SD[which.min(p.greater),], by="Geneset"]
  df_sub2 = df_sub2[p.greater > 1e-3,]

  df_sub[!(df_sub$Geneset %in% df_sub2$Geneset),][1:4,]
  })
df_gs = do.call(rbind, df_gs)

df_gs %>% kable() %>% kable_styling(bootstrap_options = "striped", full_width = FALSE)

df = df_gs[grep("CellType", coef),]
# df = rbind(df, df_gs[grep('GABAGLU_vs_Other|GABA_vs_GLU', coef)])
df[,coef := gsub("CellType", '',coef)]
df$coef = factor(df$coef, names(colorCodes))

ggplot(df, aes(Geneset,coef, -log10(p.greater))) + geom_tile() + coord_flip() + scale_y_continuous(expand=c(0,0)) + theme_classic() + ylab(bquote(-log[10]~p)) + scale_fill_manual(values = c(colorCodes, GABAGLU_vs_Other = "green"), name="Cell type") 
```
...
coefTests = colnames(coef(fitBasic))[c(1, 7:9)]
...
hcl_col = hclust(dist(M), method="ward.D")
df$coef = factor(df$coef, rownames(M)[hcl_col$order])

hcl_row = hclust(dist(t(M)), method="ward.D")
# df$Geneset = factor(df$Geneset, colnames(M)[hcl_row$order])
df$Geneset = factor(df$Geneset, cls)
df$label = ifelse(df$FDR < 0.05, '*', '')

ggplot(df, aes(coef, Geneset, fill=-log10(p.greater), label=label)) + geom_tile() + theme_classic() + scale_fill_gradient(limits=c(0, NA), low="white", high="red", name=bquote(-log[10]~P)) + theme(aspect.ratio = ncol/nrow, axis.text.x = element_text(angle = 90, vjust = 1, hjust=1)) + xlab('') + ylab('') + geom_text(vjust=1, hjust=0.5)
```
...
# idx = info$CellType %in% c("Olig", "GABA")
# isexpr = filterByExpr(geneCounts[,idx], model.matrix(~CellType, info[idx,]))
# dge = DGEList(geneCounts[isexpr,idx])
# dge = calcNormFactors(dge)
# vobj = voom(dge, model.matrix(~CellType, droplevels(info[idx,])))

# fit = lmFit( vobj, model.matrix(form, droplevels(info[idx,])))
# fit = eBayes(fit)
# fit = dream(vobj, form, info[idx,])
# fit = eBayes(fit)
# res2 = zenith(fit, coef='CellTypeOlig',idxGO )

# # Map from Ensembl genes in geneSets_GO to 
# # Ensembl names from RNA-seq data 
# index = ids2indices( recodeToList(geneSets_GO), rownames(fit))

# n_genes_in = 20
# idxGO = index[sapply(index, length) > n_genes_in]

# res = camera( vobj, idxGO, model.matrix(form, droplevels(info[idx,])), inter.gene.cor=NA, contrast='CellTypeOlig')
...
## Plots of top differentially expressed genes
```{r markerGenes, fig.width=15, fig.height=30}
...
infoExpand = rbind(data.frame(info[,c(1,3)], dataset="FANS4", stringsAsFactors=FALSE), 
  data.frame(Name=colnames(gtex_count[,id]), CellType = "GTEx BA9", dataset="GTEx", stringsAsFactors=FALSE))
rownames(infoExpand) = infoExpand$Name

# the first PC separates FANS from bulk, so subtract that and use residuals
fit = dream(geneExprCombined, ~ dataset, infoExpand)
...
xlab = paste0('PC1 (', format(varFrac[1]*100, digits=3), '%)')
ylab = paste0('PC2 (', format(varFrac[2]*100, digits=3), '%)')
ggplot(df_pca, aes(PC1, PC2, color=CellType)) + geom_point() + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + scale_color_manual("Cell Type", values=c(colorCodes, "grey")) + xlab(xlab) + ylab(ylab)

# xlab = paste0('PC2 (', format(varFrac[2]*100, digits=3), '%)')
# ylab = paste0('PC3 (', format(varFrac[3]*100, digits=3), '%)')
# ggplot(df_pca, aes(PC2, PC3, color=CellType)) + geom_point() + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + scale_color_manual("Cell Type", values=c(colorCodes, "grey")) + xlab(xlab) + ylab(ylab)
...
d = as.dist(1 - cor(geneExprCombined)^2)
# d = as.dist(1 - cor(apply(geneExprCombined, 2, RNOmni::rankNorm), method="sp")^2)

df_mds = data.frame(cmdscale( d, k=3))

df_mds = merge(df_mds, infoExpand, by.x="row.names", by.y="Name")

ggplot(df_mds, aes(X1, X2, color=CellType)) + geom_point() + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + scale_color_manual("Cell Type", values=c(colorCodes, "grey")) + xlab('Coordinate 1') + ylab('Coordinate 2')

ggplot(df_mds, aes(X2, X3, color=CellType)) + geom_point() + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + scale_color_manual("Cell Type", values=c(colorCodes, "grey")) + xlab('Coordinate 2') + ylab('Coordinate 3')
```
