...
knit2synapse::createAndKnitToFolderEntity(file = "combined_analysis/combine_all_DLPFC_by_celltype.Rmd",
                                          ...
                                          folderName = 'Cell type specific analysis of read count data by cohort',
                                          overwrite=TRUE, 
                                          knitmd=TRUE)
```
...
library(githubr)
parentId = 'syn22416298';
activityName = 'Cell type specific analysis of read count data by cohort';
activityDescription = 'Cell type specific analysis of read count data by cohort';

thisFileName <- 'combine_all_DLPFC_by_celltype.Rmd'
...
vobj.lst = lapply( levels(METADATA$Cohort), function(chrt){
  ...
  new("EList", list(E = geneExpr[,idx], weights = 1/geneExpr.se[rownames(geneExpr), colnames(geneExpr)][,idx]))
  })
...
if( params$upload ){    
  ...
  CODE <- Folder(name = "Cell type specific analysis of read count data by cohort", parentId = "syn22416298")
  CODE <- synStore(CODE)

  # Store data 
  storeData = function( name, cellType, description){
    file = paste0(name, "_", cellType, ".RDS")
    saveRDS( eval(parse(text=name)), file=file)

    nEXP_OBJ = File(file, 
                    name = paste0(cellType, ' - ',description), 
