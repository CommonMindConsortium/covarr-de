...
knit2synapse::createAndKnitToFolderEntity(file = "combined_analysis/combine_all_DLPFC_by_celltype.Rmd",
                                          ...
                                          folderName = 'Analysis of read count data by cohort: MgAs',
                                          overwrite=TRUE, 
                                          knitmd=TRUE)
```
...
vobj.lst = lapply( levels(METADATA$Cohort), function(chrt){
  ...
  idx = METADATA$Cohort == chrt

  # get variance withing cohort
...
eval_residuals = function( form ){
  ...
  residuals.lst = lapply( vobj.lst, function(vobj){
    ...
    resid.lst = fitVarPartModel(vobj, form_use, info, fxn = f, BPPARAM = SnowParam(4))  
...
fit_inter_sex.disease.lst = lapply( vobj.lst, function(vobj){
  ...
  fit = dream(vobj[keep.ensID,], form_use, info, BPPARAM = SnowParam(4))  
  eBayes( fit )
  })
