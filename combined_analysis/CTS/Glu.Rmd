...
subtitle: "Cell type specific analysis of read count data by cohort"
author:
...
rmarkdown::render("combined_analysis/CTS/Glu.Rmd")
...
synLogin()
knit2synapse::createAndKnitToFolderEntity(file = "combined_analysis/combine_all_DLPFC_by_celltype.Rmd",
                                          parentId ="syn23241817",
                                          folderName = 'Analysis of read count data by cohort: GABA',
                                          overwrite=TRUE, 
                                          knitmd=TRUE)
```
...
library(githubr)
parentId = 'syn23241817';
activityName = paste('Analysis of read count data by cohort:', params$cellType)
activityDescription = paste('Analysis of read count data by cohort:', params$cellType)

thisFileName <- paste0(ifelse(params$cellType=='GLU', 'Glu', 'GLU'), '.Rmd')
...
geneExpr = geneExpr.CTS[,params$cellType,]
geneExpr.se = geneExpr.CTS.se[,params$cellType,]
...
indToRetain = intersect(METADATA$SampleID, colnames(geneExpr))
     
geneExpr = geneExpr[,indToRetain]
     
rownames(METADATA) = METADATA$SampleID
```

```{r create.voom}
vobj.lst = lapply( levels(METADATA$Cohort), function(chrt){

  # get variance within cohort and sex
  # only keep genes with variance > 0
  # v = apply( geneExpr[,idx], 1, var)
  v = lapply( unique(METADATA$Reported_Gender), function(Gnd){
    idx = with(METADATA, (Cohort == chrt) & (Reported_Gender==Gnd))
    isVar = apply( geneExpr[,idx], 1, var) > 0
    names(isVar[isVar])
    })

  # genes with variance in both sexes
  isexpr = intersect(v[[1]], v[[2]])

  # get expressed genes form this cohort
  E = geneExpr[isexpr, METADATA$Cohort == chrt]

  new("EList", list(E = E, weights = 1/geneExpr.se[rownames(E), colnames(E)]))
  })
names(vobj.lst) = levels(METADATA$Cohort)

# only keep genes common to both cohorts
isexpr = intersect(rownames(vobj.lst[[1]]), rownames(vobj.lst[[2]]))

vobj.lst = lapply(vobj.lst, function(vobj){
  vobj[isexpr,]
  })
```
...
form.lst = list(
  `MSSM-Penn-Pitt`  = ~ Dx + RIN +scale(IntronicRate) + scale(IntragenicRate) + scale(IntergenicRate) + scale(rRNARate) + Institution*ageOfDeath ,
  `NIMH-HBCC`       = ~ Dx + RIN + scale(IntronicRate) + scale(IntragenicRate) + scale(IntergenicRate) + scale(rRNARate) + ageOfDeath)
...
if( params$upload ){    
  ...
  CODE <- Folder(name = gsub(":", " -",activityName), parentId = parentId)
  CODE <- synStore(CODE)

  # Store data 
  storeData = function( name, description){
    file = paste0(name, "_", params$cellType, ".RDS")
    saveRDS( eval(parse(text=name)), file=file)

    nEXP_OBJ = File(file, 
                    name = paste0(params$cellType, ' - ',description), 
