...
fig1 = plotVolcano( tab, rownames(tab)[1:20] ) + ggtitle("SCZ")

tab = topTable(fit_marginal, "BP", number=Inf)
fig2 = plotVolcano( tab, rownames(tab)[1:20] ) + ggtitle("BP")

plot_grid( fig1, fig2, ncol=2)

# Sex
tab = topTable(fit_marginal, "Reported_GenderMale", number=Inf) %>% getGeneSymbol
fig3 = plotVolcano( tab, rownames(tab)[1:20] ) + ggtitle("Sex")

tab2 = tab[tab$Chrom %in% 1:22,]
fig4 = plotVolcano( tab2, rownames(tab2)[1:20] ) + ggtitle("Sex")
