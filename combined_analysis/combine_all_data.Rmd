...
```{r plots.marginal}
# Disease
fig1 = plotVolcano( topTable(fit_marginal, "Scz", number=Inf) ) + ggtitle("SCZ")
fig2 = plotVolcano( topTable(fit_marginal, "BP", number=Inf) ) + ggtitle("BP")

cowplot( fig1, fig2, ncol=2)

# Sex
tab = topTable(fit_marginal, "Reported_GenderMale", number=Inf) %>% getGeneSymbol

fig3 = plotVolcano( tab ) + ggtitle("Sex")
fig4 = plotVolcano( tab[tab$Chrom %in% 1:22,] ) + ggtitle("Sex")

cowplot( fig3, fig4, ncol=2)
```
