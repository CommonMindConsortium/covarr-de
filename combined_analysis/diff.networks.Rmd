...
df_test = lapply( 1:10, function(k){
	...
	# Hypothesis testing         
	df_test = test_differential_correlation(resid.add[[resVersion]], C.diff.discovery, dynamicColors, METADATA, variable, useSLED=TRUE) 
...
beta_perm = list()
beta_perm[['MSSM-Penn-Pitt']] = beta.mle(p)
...
beta_perm[['NIMH-HBCC']] = beta.mle(p)
```
...
# Hypothesis testing         
df_test_null = test_differential_correlation(resid.add[[resVersion]], C.diff.discovery, dynamicColors, METADATA, variable, useSLED=TRUE) 
```
...
```{r testing}
# Hypothesis testing         
df_test.Sex = test_differential_correlation(resid.add[[resVersion]], C.diff.discovery, dynamicColors.Sex, METADATA, variable, useSLED=TRUE)
```
...
```{r testing.Dx}
# Hypothesis testing     
df_test.Dx = test_differential_correlation(resid.add[[resVersion]], C.diff.discovery, dynamicColors.Dx, METADATA, variable, useSLED=TRUE)  
```
