...
dynamicMods = cutreeDynamic(hcl, distM=C.diff.discovery, minClusterSize = minClusterSize)
...
hist(p, main="P-values under the null in the replication cohort", xlab="p-value")
```
...
figList = lapply( df_cohort2$Module[i], function(module){
	...
	ggplot(res, aes(Test, -log10(PValue))) + geom_bar(stat="identity", fill=res$Geneset[1]) + coord_flip() + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5))  + scale_y_continuous(expand=c(0,0)) + ggtitle(module)
}) 
plot_grid(plotlist = figList, ncol=1)
```
...
```{r plot.enrich.Dx_Sex, fig.width=9}
figList = lapply( df_cohort2$Module[i], function(module){
...
## DE enrichment
```{r cameraPR.Dx_Sex}
res_enrich_DE = enrich_module_DE( df_module_enrich.Dx_Sex )
