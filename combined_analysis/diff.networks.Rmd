...
par(mfrow=c(2,2))
lapply( unique(METADATA$Institution), function(INST){
...
```{r plot.discovery.Dx}
# plot discovery and testing.  
ggplot(df_test.Dx, aes(Module, -log10(FDR))) + geom_bar(stat='identity', fill=df_test$Module) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) + coord_flip() + facet_wrap(~Cohort) + ylab(bquote(-log[10]~FDR)) + scale_y_continuous(expand=c(0,0)) + geom_hline(yintercept = -log10(0.05), color="red", linetype="dashed")
```
...
# plot discovery and testing.  
ggplot(df_test.Dx_Sex, aes(Module, -log10(FDR))) + geom_bar(stat='identity', fill=df_test$Module) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) + coord_flip() + facet_wrap(~Cohort) + ylab(bquote(-log[10]~FDR)) + scale_y_continuous(expand=c(0,0)) + geom_hline(yintercept = -log10(0.05), color="red", linetype="dashed")
```
...
```{r modules.in.second.cohort..Dx_Sex, fig.width=8, fig.height=100}
  
i = which(df_cohort2$FDR < 0.05)[1:10]

figList = lapply(df_cohort2$Module[i], function(mod){

	plot_module( mod, df_test.Dx_Sex, METADATA, dynamicColors.Dx_Sex, C.diff.discovery, resid.add[[resVersion]], variable, "NIMH-HBCC", lvlidx=1:2)  

	plot_module( mod, df_test.Dx_Sex, METADATA, dynamicColors.Dx_Sex, C.diff.discovery, resid.add[[resVersion]], variable, "NIMH-HBCC", lvlidx=3:4) 

	plot_module( mod, df_test.Dx_Sex, METADATA, dynamicColors.Dx_Sex, C.diff.discovery, resid.add[[resVersion]], variable, "NIMH-HBCC", lvlidx=1:4) 
	})
