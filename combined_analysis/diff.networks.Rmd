...
```{r sex.confounding, fig.width=5, fig.height=8}
...
## Module enrichment
```{r top.enrich.Sex}
df_module = data.frame(	ENSEMBL = rownames(C.diff.discovery) %>% trim_ensembl_ids, 
						Module = dynamicColors.Sex)

df_module_enrich.Sex = enrich.test( df_module, go.gs, df_cohort2$Module[i])
```

```{r plot.enrich.Sex}
figList = lapply( df_cohort2$Module[i], function(module){
	plot_enrich(df_module_enrich.Sex, module) + ggtitle(module)
	})
...
df_module_enrich.Dx = enrich.test( df_module, go.gs, df_cohort2$Module[i])
```


```{r plot.enrich.Dx}
figList = lapply( df_cohort2$Module[i], function(module){
	plot_enrich(df_module_enrich.Dx, module) + ggtitle(module)
	})
plot_grid(plotlist = figList, ncol=2)
```
...
df_module_enrich.Dx_Sex = enrich.test( df_module, go.gs, df_cohort2$Module[i])
```


```{r plot.enrich.Dx_Sex}
figList = lapply( df_cohort2$Module[i], function(module){
	plot_enrich(df_module_enrich.Dx_Sex, module) + ggtitle(module)
	})
plot_grid(plotlist = figList, ncol=2)
```
