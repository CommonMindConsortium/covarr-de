...
df_test = lapply( 1:10, function(k){
	...
	#----------------------------------
	C = diff.net[[1]]
	C.diff.discovery = C[[2]] - C[[1]]
	C.diff.discovery = C.diff.discovery[[1]][keep.ensID,keep.ensID]
...
df_test_recal = apply_permutation_null( df_test.Sex )
...
```{r testing.Dx_Sex}
# Hypothesis testing     
df_test.Dx_Sex = test_differential_correlation_interaction(resid.add[[resVersion]], C.diff.discovery, dynamicColors.Dx_Sex, METADATA, nperm=20)    
```
...
df_test_recal = apply_permutation_null( df_test.Dx_Sex )

table(df_test_recal$FDR < 0.05) 

df_test_meta = module_meta_analysis( df_test_recal )

table(df_test_meta$FDR < 0.051)
```
