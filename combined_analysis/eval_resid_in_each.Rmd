...
for( datID in names(dge.lst) ){
  ...
  # for each cell type
  for( i in length(dimnames(geneExpr)[[2]]) ){
    ...
    PEER_setPhenoMean(model, t(geneExpr[1:100,i,]))
    ...
    storeData( vobj$E, file, paste0(datID, " Observed expression", '_', CT,))

    # upload covariate matrix
    file = paste0('covariateMatrix_',gsub(" ", '', gsub("-", "_", datID)), '_', CT, ".RDS")
    storeData( cov_matrix, file, paste0(datID, " Covariates for QTL analysis", '_', CT,))

    # upload latent variables
    file = paste0('PEER',gsub(" ", '', gsub("-", "_", datID)), '_', CT, ".RDS")
    storeData( latentVariables, file, paste0(datID, " Latent variables for QTL analysis", '_', CT,))
  }
}
```
