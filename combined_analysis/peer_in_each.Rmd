...
library(githubr)
parentId = 'syn22012942';
activityName = 'Run PEER in each dataset';
activityDescription = 'Run PEER in each dataset';

thisFileName <- 'peer_in_each.Rmd'
...
METADATA %>% group_by(Dx, Institution, Reported_Gender, Region) %>% dplyr::summarise(count = n()) %>% 
kable(caption="Diagnosis, Institution, Sex, Region") %>% kable_styling(full_width=FALSE) 
```

```{r}
CODE <- Folder(name = "Run PEER in each dataset", parentId = "syn22012942")
CODE <- synStore(CODE)

# Store data 
storeData = function( name, file, description){
...
dge.lst = lapply( 1:nrow(grid), function(i){
  ...
  if( sum(idx) == 0){
    ...
    res = calcNormFactors(dge, "none")
  }
  res
})
names(dge.lst) = apply(grid, 1, paste, collapse=' - ')
dge.lst = dge.lst[!is.null(sapply(dge.lst, nrow))]
...
form = ~ Dx + Reported_Gender + RIN + scale(IntronicRate) + scale(IntragenicRate) + scale(IntergenicRate) + scale(rRNARate) + ageOfDeath + cellFrac_ilr_1 + cellFrac_ilr_2 + cellFrac_ilr_3


residMat.lst = lapply( names(dge.lst), function(datID){
  ...
  # apply voom
  vobj = voomWithDreamWeights(dge[1:100,], form_use, info, save.plot=TRUE, BPPARAM = SnowParam(18))

  # Compute residuals, but add back in Dx*Reported_Gender
  # for form = ~ Dx*Reported_Gender
  f = function(fit){
    residuals(fit) + get_prediction( fit, ~ Dx + Reported_Gender + cellFrac_ilr_1 + cellFrac_ilr_2 + cellFrac_ilr_3)
  }
