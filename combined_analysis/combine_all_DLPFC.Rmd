...
form = ~ ageOfDeath:Institution + Reported_Gender + Dx + Institution + Dx

Y = clr(METADATA[,CTs])

vp = fitExtractVarPartModel( t(Y), form, METADATA, BPPARAM=SerialParam())
plotPercentBars(vp) + ggtitle("Explain cell type composition")

ymax = max(rowSums(vp[,-ncol(vp)]))*110
plotPercentBars(vp) + scale_y_continuous(expand=c(0,0), limits=c(0, ymax))
```




```{r sex.graph, fig.width=7, fig.height=7}
library(ggplot2)
library(ggraph)
library(tidygraph)
...
# Dx test with in Males and then Females
