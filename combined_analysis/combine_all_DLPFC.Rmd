...
ggplot(METADATA, aes(Reported_Gender, ageOfDeath)) + geom_boxplot() + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + facet_wrap(~Institution, nrow=1)
```

### Sex by cell fraction
```{r summary.sex, fig.width=8, fig.height=10}
figList = lapply(colnames(df_cellFractions), function(CT){
  ggplot(METADATA, aes_string('Reported_Gender', CT)) + geom_boxplot() + theme_bw() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + facet_wrap(~Institution, nrow=1)
})
...
fig =  plot_forrest( c(tab.de.lst[[coef]], list("Meta-analysis" = df_meta[[coef]])), geneSymbol)
...
show_zenith_results( res_meta_zenith.sex_autosome, "Reported_GenderMale", "GO", FDR=0.05) 
show_zenith_results( res_meta_zenith.sex_autosome, "Reported_GenderMale", "MSigDB", FDR=0.05)   
```
...
# DIFFERENTIAL CORRLEATION NETWORKS
# CLUSTER on C_Scz - C_Control
# Like contrastive PCA
