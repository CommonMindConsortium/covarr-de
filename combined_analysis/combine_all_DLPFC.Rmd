...
rstat_nodes <- data.frame(name = unique(df_cca2$v1))
...
ggraph(net,  layout = 'igraph', algorithm = 'star') + 
  ...
  scale_edge_color_gradient(name=bquote(R^2), low="white", high="red", lim=c(0, max(df_cca2$V_sqrt^2))) +
  ggtitle("Pairwise squared correlation")
```
...
i = with(res_meta_zenith, (Resource == 'GO') & (Coef == "DxSCZ"))
df = res_meta_zenith[i,]
df$qvalue = qvalue(df$PValue)$qvalue
# df$p.adjust = p.adjust(df$PValue, method="bon" )
df$p.adjust = pmin(1, 2805 * df$PValue)

M = recodeToSparseMatrix(gs.GO)
M = M[rownames(M) %in% trim_ensembl_ids(rownames(df_meta_SCZ)),]
keep = (colSums(M) >= 10) & (colnames(M) %in% rownames(df) )
M = M[,keep]



m = colMeans(M)
s = apply(M, 2, var) * nrow(M) / (nrow(M) + 1)

dcmp = irlba( M, nv = 500, center=m, scale=sqrt(s), right_only=FALSE)
ev = dcmp$d^2 / nrow(M)

plot(cumsum(ev) / ncol(M))



df_unsort = df[colnames(M),]
edcmp = list(vectors = dcmp$v, values = dcmp$d^2 / nrow(M))

res = p.corr.adjust( df_unsort$PValue, dcmp = edcmp, method.sim="eigen", B = nrow(df_unsort)*4)



plot( cumsum(dcmp$d^2 / sum(dcmp$d^2)))

C = cor(as.matrix(M[,1:1000]))
evals = eigen(C)$values

dcmp = svd(scale(M) / sqrt(nrow(M)))
dcmp$d^2
...
dcmp = svd(scale(M[,1:100]))


meff(eigen = dcmp$d^2 , method = "galwey")
...
```{r plot.gene_set_test.marginal, eval=TRUE}
 plot_zenith_results = function( res, coef, Resource, FDR = 0.05){
  ...
  ymax = max(-log10(df$PValue)) * 1.05
  ggplot(df, aes(Set, -log10(PValue))) + geom_bar(stat="identity", fill="navy") + theme_bw() + theme(aspect.ratio=1,plot.title = element_text(hjust = 0.5)) + coord_flip() + ylab(bquote(-log[10]~P)) + xlab('') + ggtitle(coef) + scale_y_continuous(expand=c(0,0), limits=c(0, ymax))
}
...
# Concordance analysis
resConcord.Sex = plot_concordance(tab.de.lst.autosome, showGenes = tab.de.lst.autosome$Symbol[1:20])
...
```{r compare.separate, fig.width=5, fig.height=5}
df = merge( df_meta.separ[[1]], df_meta.separ[[2]], by="row.names")

fit = with(df, lm(logFC.y ~ logFC.x))

coef(summary(fit) ) %>%  kable(caption="logFC is larger in males") %>% kable_styling(full_width=FALSE) 

 # compare logFC
tstat = with(df, (logFC.y - logFC.x) / (sqrt(se.y^2 + se.x^2)))

qqt(tstat)
abline(0, 1, col="red")
p = 2*pt(abs(tstat), df=778, lower.tail=FALSE)
table(p.adjust(p) < 0.05)
```
